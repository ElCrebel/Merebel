...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Merebel Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";

        if (mapArgs.count("-version")) {
            strUsage += LicenseInfo();
        } else {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  Merebeld [options]                     " + _("Start Merebel Core Daemon") + "\n";
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Merebel:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in Merebeld anymore. Use the Merebel-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
...
int main(int argc, char* argv[])
{
    ...
    // Connect Merebeld signal handlers
    noui_connect();
