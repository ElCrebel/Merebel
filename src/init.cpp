...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "Merebel2.conf"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 22777, 7773));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, Merebel, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-backupzMerebel=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMerebel minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeMerebelamount=<n>     " + strprintf(_("Keep N MERI anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 7773, 38843));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Merebel Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZMerebelBackups = GetBoolArg("-backupzMerebel", true);
        pwalletMain->setZMerebelAutoBackups(fEnableZMerebelBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeMerebelAmount = GetArg("-anonymizeMerebelamount", 0);
//    if (nAnonymizeMerebelAmount > 999999) nAnonymizeMerebelAmount = 999999;
//    if (nAnonymizeMerebelAmount < 2) nAnonymizeMerebelAmount = 2;
    ...
    LogPrintf("Anonymize Merebel Amount %d\n", nAnonymizeMerebelAmount);
