// Copyright (c) 2018-2019 The Gentarium Core developers
...
void CDevBudget::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    if (txNew.vout[i - 1].nValue > 0) {
        ...
        if(budgetPayment > 0) {
            ...
            LogPrintf("MN reward: %s\n",FormatMoney(txNew.vout[i - 1].nValue).c_str());
            //subtract budget payment from mn reward
            txNew.vout[i - 1].nValue -= budgetPayment;

            LogPrintf("Dev budget payment of %s to %s MN reward %s\n", FormatMoney(budgetPayment).c_str(), Params().DevRewardAddress().c_str(),FormatMoney(txNew.vout[i - 1].nValue).c_str());
        }
        else{
