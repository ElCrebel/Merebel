...
// Copyright (c) 2017-2020 The Merebel developers
...
{
    // before stake modifier V2, the age required was 60 * 60 *24 (24 hour) / not required on regtest
    if (!IsStakeModifierV2(contextHeight))
        return (NetworkID() == CBaseChainParams::REGTEST || (utxoFromBlockTime + 60 * 60 * 24 <= contextTime));
...
*/
       ...
       vSeeds.push_back(CDNSSeedData("seedsmeri.minnodes.io", "seedsmeri.minnodes.io"));
        ...
        nZerocoinRequiredStakeDepth = 200; //The required confirmations for a zmeri to be stakable
...
public:
    CTestNetParams()
    {
        ...
        nMinerThreads = 0;
        nTargetSpacing = 1 * 30; // Merebel: half minute
        nLastPOWBlock = 350;
        nMaturity = 15;
	    nStakeMinDepth = 100;
        nFutureTimeDriftPoW = 7200;
	    ...
	    hashGenesisBlock = genesis.GetHash();
        assert(hashGenesisBlock == uint256("0x0000f6c9d5ee60d74d44aa8723972a8b017775322c8accaf8891e0bbe927730d"));
