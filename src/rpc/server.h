...
/**
 * MEREBEL RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZmeriSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzmeriseed(const UniValue& params, bool fHelp);
extern UniValue getzmeriseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzmeri(const UniValue& params, bool fHelp);
extern UniValue dzmeristate(const UniValue& params, bool fHelp);
