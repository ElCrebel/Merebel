...
#include "ui_zMerebelcontroldialog.h"
...
std::list<std::string> ZMerebelControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMerebelControlDialog::listMints;

ZMerebelControlDialog::ZMerebelControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMerebelControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMerebelControlDialog::~ZMerebelControlDialog()
{
    delete ui;
}

void ZMerebelControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMerebelControlDialog::updateList()
{
...
void ZMerebelControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMerebelControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMerebel_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMerebelControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMerebelControlDialog::GetSelectedMints()
{
...
void ZMerebelControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
