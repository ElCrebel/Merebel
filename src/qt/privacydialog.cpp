...
#include "zMerebelcontroldialog.h"
#include "spork.h"
...
void PrivacyDialog::on_pushButtonZMerebelControl_clicked()
{
    ZMerebelControlDialog* zMerebelControl = new ZMerebelControlDialog(this);
    zMerebelControl->setModel(walletModel);
    zMerebelControl->exec();
}

void PrivacyDialog::setZMerebelControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMerebelSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMERI()
{
    ...
    // use mints from zMerebel selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMerebelControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMerebelControlDialog::GetSelectedMints();
    }
    ...
    // Clear zMerebel selector in case it was used
    ZMerebelControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMerebel Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Merebel, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMerebel Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzMERIAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
