...
#include "ui_zmericontroldialog.h"
...
std::set<std::string> ZMeriControlDialog::setSelectedMints;
std::set<CMintMeta> ZMeriControlDialog::setMints;

bool CZMeriControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZMeriControlDialog::COLUMN_DENOMINATION || column == ZMeriControlDialog::COLUMN_VERSION || column == ZMeriControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZMeriControlDialog::ZMeriControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMeriControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMeriControlDialog::~ZMeriControlDialog()
{
    delete ui;
}

void ZMeriControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMeriControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZMeriControlWidgetItem* itemDenom(new CZMeriControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZMeriControlWidgetItem *itemMint = new CZMeriControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zmeriTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zmeriTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zMERI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zMERI seed used to mint this zMERI is not the same as currently hold in the wallet";
            else
...
void ZMeriControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMeriControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMeri_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMeriControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMeriControlDialog::GetSelectedMints()
{
...
void ZMeriControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
