...
CzMERIWallet::CzMERIWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmeri seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMERISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMERISeed_deprecated()) {
                    LogPrintf("%s: Updated zMERI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmeri seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmeri wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMERIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMERICount(nCountLastUsed);
    else if (!walletdb.ReadZMERICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMERIWallet::Lock()
{
    seedMaster = 0;
}

void CzMERIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMERIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMERI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMERIWallet::LoadMintPoolFromDB()
{
...
void CzMERIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMERIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMERIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmeriTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMERIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMERI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmeriTracker which also adds to database
    pwalletMain->zmeriTracker->Add(dMint, true);
...
void CzMERIWallet::SeedToZMERI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMERIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMERIWallet::GenerateDeterministicZMERI(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMERIWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMERIWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMERIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
