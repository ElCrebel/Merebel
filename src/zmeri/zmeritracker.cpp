// Copyright (c) 2018-2019 The MEREBEL developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zmeri/deterministicmint.h>
#include "zmeritracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzMERITracker::CzMERITracker(std::string strWalletFile)
{
...
CzMERITracker::~CzMERITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMERITracker::Init()
{
...
bool CzMERITracker::Archive(CMintMeta& meta)
{
...
bool CzMERITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMERITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMERITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMERITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzMERITracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzMERITracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzMERITracker::GetSerialHashes()
{
...
CAmount CzMERITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMERITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMERITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMERITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMERITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMERITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMERITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMERITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMERITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMERITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMERITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMERIWallet* zMERIWallet)
{
    bool iszMERIWalletInitialized = (NULL != zMERIWallet);
    CMintMeta meta;
    ...
    if (! iszMERIWalletInitialized)
        zMERIWallet = new CzMERIWallet(strWalletFile);
    meta.isSeedCorrect = zMERIWallet->CheckSeed(dMint);
    if (! iszMERIWalletInitialized)
        delete zMERIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMERITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMERITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMERITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMERITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMERITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMERITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMERIWallet* zMERIWallet = new CzMERIWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMERIWallet);
        }
        delete zMERIWallet;
        LogPrint("zero", "%s: added %d dzmeri from DB\n", __func__, listDeterministicDB.size());
    }
